...
import {IBrand, ICategory, IItem, IOrder, IOrderItem, ISize, IType, IVariant} from "../../utils/models";
...
export const addBranch = (name: string) => async(dispatch: AppDispatch) => {
    dispatch(userSlice.actions.startAction())
    try {
        await $authHost.post('api/branch', {name})
        dispatch(userSlice.actions.setSuccess('Филиал успешно добавлен !'))
    } catch (e) {
        dispatch(userSlice.actions.setError('Не получилось добавить категорию !'))
    }
}
...
export const addSize = ({itemId, branchId, name, quantity , purchase, branchName}:{branchName: string | undefined, itemId: number, branchId: number, name: string, quantity: string, purchase: string} ) => async(dispatch: AppDispatch) => {
    dispatch(userSlice.actions.startAction())
    try {
        await $authHost.post('api/size', {branchName, itemId, branchId, name, quantity, purchase })
...
export const createOrder = ({orderData, orderItems}: {orderData: IOrder, orderItems: IOrderItem[]}) => async(dispatch: AppDispatch) => {
    dispatch(userSlice.actions.startAction())
    try {
        await $host.post('api/order', {orderData,orderItems})
...
export const createCompilation = ({name, image}: {name: string, image: any}) => async(dispatch: AppDispatch) => {
    dispatch(userSlice.actions.startAction())
    try {
        const formData = new FormData()
        let f = await imageCompression(image,
            {maxSizeMB: 1, maxWidthOrHeight: 1920, useWebWorker: true}
        )
        formData.append('image', f)
        formData.append('name', name)
        await $authHost.post('api/compilation', formData)
        dispatch(userSlice.actions.setSuccess('Подборка добавлена !'))
    } catch (e) {
        dispatch(userSlice.actions.setError('Подборка не добавлена !'))
    }
}
export const createExpense = ({branchId, expenseSum, expenseName, expenseDate}: {branchId: number, expenseSum: string, expenseName: string, expenseDate: string}) => async(dispatch: AppDispatch) => {
    dispatch(userSlice.actions.startAction())
    try {
        await $authHost.post('api/expenses', {branchId, expenseSum, expenseName, expenseDate})
...
export const createDay = ({name, branchId}: {name: string, branchId: number}) => async(dispatch: AppDispatch) => {
    dispatch(userSlice.actions.startAction())
    try {
        await $authHost.post('api/day', {name, branchId})
        dispatch(userSlice.actions.setSuccess('Расход добавлен !'))
    } catch (e) {
        dispatch(userSlice.actions.setError('Не получилось добавить расход !'))
    }
