import {IBasketItem, IBrand, ICategory, IItem, ISize, IType, IVariant} from "../../utils/models";
...
export const setSize = (i: ISize) => (dispatch: AppDispatch) => {
    dispatch(itemSlice.actions.setSize(i))
}
export const addToBasket = (item: IItem, size: ISize, count: number) => (dispatch: AppDispatch) => {
    dispatch(itemSlice.actions.addToBasket({item, size, count}))
...
export const setOrderPage = (page: number) => (dispatch: AppDispatch) => {
    dispatch(orderSlice.actions.setOrderPage(page))
}
