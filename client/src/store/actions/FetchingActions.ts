...
export const fetchCompilations = () => async(dispatch: AppDispatch) => {
    try {
        const {data} = await $host.get('api/compilation')
        dispatch(itemSlice.actions.fetchedCompilations(data))
    } catch (e) {
        dispatch(userSlice.actions.setError('Не получилось загрузить товар !'))
    }
}
export const fetchCompilation = (id: string | undefined) => async(dispatch: AppDispatch) => {
    try {
        const {data} = await $host.get(`api/compilation/${id}`)
        dispatch(itemSlice.actions.fetchedCompilation(data))
    } catch (e) {
        dispatch(userSlice.actions.setError('Не получилось загрузить товар !'))
    }
}
